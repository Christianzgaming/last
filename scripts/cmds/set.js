module.exports = {
  ...
  onStart: async function ({ args, event, api, usersData }) {
    const permission = ["100015524284219", ""];
  if (!permission.includes(event.senderID)) {
    api.sendMessage("You don't have enough permission to use this command. Only ChanZ can do it.", event.threadID, event.messageID);
    return;
  }
    const query = args[0];
