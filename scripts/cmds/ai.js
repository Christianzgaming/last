...
module.exports = {
  ...
  onChat: async function ({ api, event, args, message }) {
    try {
      ...
      const answerWithHeader = getCenteredHeader("Master Christian AI 💬") + "\n━━━━━━━━━━━━\n" + answer;
...
function getCenteredHeader(header) {
  const totalWidth = 62; // Adjust the total width as needed
...
async function getGPTResponse(prompt) {
  ...
  const response = await axios.get(`${GPT_API_URL}?query=${encodeURIComponent(prompt)}`);
